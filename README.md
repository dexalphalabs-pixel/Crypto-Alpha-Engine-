# Crypto Alpha Engine v0.7.7 Data Source Reliability
...
- Dodano test Telegrama w GitHub Actions przed skanem.
- Dodano fallback CEX przez OKX, gdy Binance zwraca HTTP 451.
- Dodano fallback Market Context przez CoinGecko dla BTC/ETH.
- Dodano jawne SEND_TELEGRAM_* w workflow.
- Dodano tools/test_telegram.py.


## v0.7.7 Telegram summary

Summary na Telegramie pokazuje teraz nie tylko liczbę alertów, ale też krótkie dane o tym, co jest warte uwagi: top alerty, top watchlist, powody i ryzyka. Harmonogram działa co godzinę na minucie 17 (`17 * * * *`), bo GitHub Actions bywa opóźniony na pełnej godzinie.


## v0.7.7 Data Source Reliability

- Dodano Bybit jako drugi fallback CEX po OKX.
- Market Context korzysta z fallback chain: Binance → CoinGecko → Bybit.
- Telegram summary pokazuje tryb działania: FULL / DEX_ONLY / DEX_ONLY_DEGRADED / CEX_ONLY / DEGRADED.
- Performance z dużym opóźnieniem jest oznaczany w Telegramie jako STALE.
- Primary-source failures są traktowane jako warnings, jeśli fallback dostarczył dane.


## v0.7.7 Source Intelligence

Ta wersja dodaje ocenę wiarygodności źródeł: `source_confidence`, `source_coverage_score` i provider matrix. Mocne alerty nie powinny bazować wyłącznie na wysokim score tokena — wymagają także potwierdzenia źródłowego.

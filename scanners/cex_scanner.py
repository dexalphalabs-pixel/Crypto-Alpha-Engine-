...
class CexScanner:
    ...
    def scan(self) -> list[Candidate]:
        try:
            ...
            msg = str(exc)
            self.health["warnings"].append(f"Binance primary unavailable: {msg}")
            log.warning("Binance CEX scan failed; trying fallbacks: %s", msg)

        for provider in self.settings.cex_fallback_order:
            provider = provider.lower().strip()
            if provider == "okx" and self.settings.okx_enabled:
                try:
                    return self._scan_okx_fallback()
                except Exception as exc:
                    self.health["warnings"].append(f"OKX fallback unavailable: {exc}")
                    log.warning("OKX CEX fallback failed: %s", exc)
            elif provider == "bybit" and self.settings.bybit_enabled:
                try:
                    return self._scan_bybit_fallback()
                except Exception as exc:
                    self.health["warnings"].append(f"Bybit fallback unavailable: {exc}")
                    log.warning("Bybit CEX fallback failed: %s", exc)

        self.health["source"] = "CEX/Fallbacks"
        self.health["status"] = "FAILED"
        self.health["errors"].append("All CEX sources failed: Binance + configured fallbacks")
        return []
    ...
    def _scan_binance(self) -> list[Candidate]:
        ...
        if not isinstance(books, list):
            self.health["warnings"].append(f"Unexpected Binance book response type: {type(books).__name__}")
            books = []

        self.health["source"] = "CEX/Binance"
        self.health["fallback_used"] = None
        ...
        for ticker in prefiltered[: self.settings.cex_klines_prefilter_limit]:
            if self.breaker.is_open:
                ...
                self.health["warnings"].append("circuit breaker open: skipped remaining Binance klines/futures checks")
                break
    ...
    def _scan_okx_fallback(self) -> list[Candidate]:
        ...
        self.health["fallback_chain"].append("OKX")
    ...
    def _scan_bybit_fallback(self) -> list[Candidate]:
        self.health["source"] = "CEX/BybitFallback"
        self.health["fallback_used"] = "Bybit"
        self.health["fallback_chain"].append("Bybit")
        self.breaker = CircuitBreaker(self.settings.api_circuit_breaker_failures)
        resp = get_json(self.session, BYBIT_TICKERS_URL, params={"category": "spot"}, timeout=(5, 18))
        result = resp.get("result", {}) if isinstance(resp, dict) else {}
        rows = result.get("list", []) if isinstance(result, dict) else []
        if not isinstance(rows, list):
            raise RuntimeError(f"Unexpected Bybit ticker response type: {type(rows).__name__}")
        self.health["tickers_raw"] = len(rows)
        self.health["books_raw"] = len(rows)
        benchmark = self._bybit_benchmark_changes(rows)
        filtered = self._prefilter_bybit(rows)
        self.health["prefiltered"] = len(filtered)
        candidates: list[Candidate] = []
        for row in filtered[: self.settings.cex_klines_prefilter_limit]:
            if self.breaker.is_open:
                self.health["circuit_open"] = True
                break
            symbol = row.get("symbol", "")
            quote = next((q for q in self.settings.cex_quotes if symbol.endswith(q)), "USDT")
            base = symbol[:-len(quote)] if quote and symbol.endswith(quote) else symbol
            last = safe_float(row.get("lastPrice"))
            change_24h = safe_float(row.get("price24hPcnt")) * 100
            bid = safe_float(row.get("bid1Price"))
            ask = safe_float(row.get("ask1Price"))
            spread_pct = ((ask - bid) / ((ask + bid) / 2)) * 100 if bid > 0 and ask > 0 else None
            if spread_pct is not None and spread_pct > self.settings.cex_max_spread_pct * 3:
                continue
            change_1h, change_4h, vol_change_1h, vol_change_4h, klines_available = self._bybit_klines_momentum(symbol)
            volume_usd = safe_float(row.get("turnover24h")) or safe_float(row.get("volume24h")) * last
            c = Candidate(
                source="CEX",
                symbol=symbol,
                name=symbol,
                exchange="Bybit",
                url=f"https://www.bybit.com/en/trade/spot/{base}/{quote}",
                price_usd=last,
                ...
                security_verified=True,
                security_score=82.0,
                security_source="bybit_listing_proxy",
                raw=row,
            )
            c.reasons.append("CEX fallback source: Bybit public market API")
            candidates.append(c)
        candidates.sort(key=lambda x: (x.volume_24h_usd, abs(x.price_change_4h_pct), abs(x.price_change_24h_pct)), reverse=True)
        candidates = candidates[: self.settings.cex_top_limit]
        self.health["candidates"] = len(candidates)
    ...
    def _prefilter_bybit(self, rows: list[dict]) -> list[dict]:
        out = []
        for row in rows:
            ...
            if not quote or symbol in {"BTCUSDT", "ETHUSDT"}:
                continue
            last = safe_float(row.get("lastPrice"))
            volume = safe_float(row.get("turnover24h")) or safe_float(row.get("volume24h")) * last
            if volume < self.settings.cex_min_quote_volume_usd:
                continue
            change_24h = abs(safe_float(row.get("price24hPcnt")) * 100)
            r = dict(row)
            r["_rough_score"] = volume + change_24h * 2_000_000
            out.append(r)
        out.sort(key=lambda r: safe_float(r.get("_rough_score")), reverse=True)
        return out[: max(self.settings.cex_top_limit, self.settings.cex_klines_prefilter_limit)]
    ...
    def _bybit_benchmark_changes(self, rows: list[dict]) -> dict[str, float | None]:
        out = {"BTC": None, "ETH": None}
        for row in rows:
            symbol = row.get("symbol") if isinstance(row, dict) else None
            if symbol == "BTCUSDT":
                out["BTC"] = safe_float(row.get("price24hPcnt")) * 100
            elif symbol == "ETHUSDT":
                out["ETH"] = safe_float(row.get("price24hPcnt")) * 100
        return out
    ...
    def _binance_klines_momentum(self, symbol: str) -> tuple[float, float, float | None, float | None, bool]:
        ...
        try:
            ...
            if len(self.health["warnings"]) < 10:
                self.health["warnings"].append(f"klines {symbol}: {exc}")
            return 0.0, 0.0, None, None, False
    ...
    def _okx_klines_momentum(self, inst_id: str) -> tuple[float, float, float | None, float | None, bool]:
        ...
        try:
            ...
            self.breaker.record_failure()
            if len(self.health["warnings"]) < 10:
                self.health["warnings"].append(f"OKX candles {inst_id}: {exc}")
            return 0.0, 0.0, None, None, False

    def _bybit_klines_momentum(self, symbol: str) -> tuple[float, float, float | None, float | None, bool]:
        if self.health["klines_checked"] >= self.settings.cex_klines_limit:
            return 0.0, 0.0, None, None, False
        self.health["klines_checked"] += 1
        try:
            resp = get_json(self.session, BYBIT_KLINES_URL, params={"category": "spot", "symbol": symbol, "interval": "60", "limit": 6}, timeout=(5, 12))
            result = resp.get("result", {}) if isinstance(resp, dict) else {}
            data = result.get("list", []) if isinstance(result, dict) else []
            self.breaker.record_success()
            # Bybit returns newest first: [start, open, high, low, close, volume, turnover]
            return self._momentum_from_klines(data, close_idx=4, vol_idx=6, reverse=True)
        except Exception as exc:
            self.breaker.record_failure()
            if len(self.health["warnings"]) < 10:
                self.health["warnings"].append(f"Bybit candles {symbol}: {exc}")
            return 0.0, 0.0, None, None, False

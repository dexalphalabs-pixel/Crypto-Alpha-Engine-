...
from engines.source_intelligence_engine import SourceIntelligenceEngine
...
def _market_sources_failed(health: list[dict]) -> bool:
    """Return True only when primary market-data layers all failed.

    v0.7.7 includes fallback source names so fast-fail remains correct after
    Binance 451 fallback handling. DEGRADED sources are not treated as hard
    failures, because they may still provide partial context.
    """
    market_sources = {
        "DEX/DexScreener",
        "CEX/Binance",
    ...
    market = [h for h in health if h.get("source") in market_sources]
    return bool(market) and all(h.get("status") == "FAILED" for h in market)


def _operational_mode(health: list[dict]) -> str:
    dex_ok = any(h.get("source") == "DEX/DexScreener" and h.get("status") in {"OK", "DEGRADED"} for h in health)
    cex_ok = any(h.get("source") in {"CEX/Binance", "CEX/OKXFallback", "CEX/BybitFallback"} and h.get("status") in {"OK", "DEGRADED"} for h in health)
    context_ok = any(h.get("source") in {"MarketContext/Binance", "MarketContext/CoinGeckoFallback", "MarketContext/BybitFallback"} and h.get("status") in {"OK", "DEGRADED"} for h in health)
    if dex_ok and cex_ok and context_ok:
        return "FULL"
    if dex_ok and not cex_ok:
        return "DEX_ONLY" if context_ok else "DEX_ONLY_DEGRADED"
    if cex_ok and not dex_ok:
        return "CEX_ONLY" if context_ok else "CEX_ONLY_DEGRADED"
    return "DEGRADED"


def _source_aware_operational_mode(mode: str, coverage: dict) -> str:
    score = coverage.get("coverage_score", 0) or 0
    if mode == "FULL" and score < 70:
        return "BROAD_BUT_DEGRADED"
    if mode in {"DEX_ONLY", "DEX_ONLY_DEGRADED"} and score < 50:
        return "LOW_CONFIDENCE_DEX_ONLY"
    if score < 35:
...
def run() -> None:
    ...
    log.info("Crypto Alpha Engine v0.7.7 start | run_id=%s | SAFE_MODE=%s | RUN_MODE=%s", run_id, settings.safe_mode, settings.run_mode)
    ...
    source_intel = None
    if settings.source_intelligence_enabled:
        source_intel = SourceIntelligenceEngine(settings)
        scored = source_intel.enrich(scored, health)
        health.append(source_intel.health)
        scored = [scoring.refresh_status(c) for c in scored]
    ...
    diagnostics["operational_mode"] = _operational_mode(health)
    if source_intel is not None:
        diagnostics.update(source_intel.diagnostics())
        diagnostics["operational_mode"] = _source_aware_operational_mode(diagnostics["operational_mode"], diagnostics.get("source_coverage_summary", {}))
    ...
    if notifier.enabled and settings.send_telegram_summary:
        try:
            ...
            notifier.send(notifier.format_summary(run_id, len(scored), len(alerts), len(watchlist), rejected, paths["html"], health=health, performance_summary=perf_summary, run_url=settings.github_run_url, alert_candidates=alerts, watchlist_candidates=watchlist, operational_mode=diagnostics.get("operational_mode", "UNKNOWN")))
        except Exception as exc:

...
def _market_sources_failed(health: list[dict]) -> bool:
    ...
    v0.7.5 includes fallback source names so fast-fail remains correct after
...
def run() -> None:
    ...
    log.info("Crypto Alpha Engine v0.7.5 start | run_id=%s | SAFE_MODE=%s | RUN_MODE=%s", run_id, settings.safe_mode, settings.run_mode)
    ...
    if notifier.enabled and settings.send_telegram_summary:
        try:
            ...
            notifier.send(notifier.format_summary(run_id, len(scored), len(alerts), len(watchlist), rejected, paths["html"], health=health, performance_summary=perf_summary, run_url=settings.github_run_url, alert_candidates=alerts, watchlist_candidates=watchlist))
        except Exception as exc:

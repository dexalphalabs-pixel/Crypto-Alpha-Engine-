...
class ScoringEngine:
    ...
    def _confidence_rank(self, level: str) -> int:
        return {"NO_CONFIDENCE": 0, "LOW": 1, "MEDIUM": 2, "HIGH": 3}.get((level or "UNKNOWN").upper(), 0)

    def _meets_confidence(self, c, required: str) -> bool:
        # Before SourceIntelligence runs, do not block. After it runs, UNKNOWN should be treated as incomplete.
        current = getattr(c, "source_confidence", "UNKNOWN")
        if current == "UNKNOWN":
            return True
        return self._confidence_rank(current) >= self._confidence_rank(required)
    ...
    def _status(self, c):
        ...
        if c.final_score >= alert_threshold and c.risk_score <= alert_risk_cap:
            # Source Intelligence gate: high scores do not equal high confidence.
            if not self._meets_confidence(c, self.settings.min_source_confidence_for_momentum_alert):
                return "WATCHLIST"
            ...
            if c.final_score >= high_conviction_threshold and c.risk_score <= 35 and self._meets_confidence(c, self.settings.min_source_confidence_for_high_conviction):

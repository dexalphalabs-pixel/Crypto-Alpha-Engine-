...
class Settings:
    ...
    cex_fallback_order: list[str] = field(default_factory=lambda: _csv("CEX_FALLBACK_ORDER", "okx,bybit"))
    bybit_enabled: bool = field(default_factory=lambda: _bool("BYBIT_ENABLED", True))
    okx_enabled: bool = field(default_factory=lambda: _bool("OKX_ENABLED", True))
    ...
    source_intelligence_enabled: bool = field(default_factory=lambda: _bool("SOURCE_INTELLIGENCE_ENABLED", True))
    min_source_coverage_score: float = field(default_factory=lambda: _float("MIN_SOURCE_COVERAGE_SCORE", 45.0))
    min_source_confidence_for_high_conviction: str = field(default_factory=lambda: os.getenv("MIN_SOURCE_CONFIDENCE_FOR_HIGH_CONVICTION", "HIGH").strip().upper())
    min_source_confidence_for_momentum_alert: str = field(default_factory=lambda: os.getenv("MIN_SOURCE_CONFIDENCE_FOR_MOMENTUM_ALERT", "MEDIUM").strip().upper())
    source_confidence_low_score: float = field(default_factory=lambda: _float("SOURCE_CONFIDENCE_LOW_SCORE", 30.0))
    source_confidence_medium_score: float = field(default_factory=lambda: _float("SOURCE_CONFIDENCE_MEDIUM_SCORE", 55.0))
    source_confidence_high_score: float = field(default_factory=lambda: _float("SOURCE_CONFIDENCE_HIGH_SCORE", 78.0))
    min_security_score_for_high_confidence: float = field(default_factory=lambda: _float("MIN_SECURITY_SCORE_FOR_HIGH_CONFIDENCE", 75.0))
    source_confidence_security_required_for_medium: bool = field(default_factory=lambda: _bool("SOURCE_CONFIDENCE_SECURITY_REQUIRED_FOR_MEDIUM", True))
    ...
    def validate(self) -> None:
        ...
        if self.min_source_confidence_for_high_conviction not in {"HIGH", "MEDIUM", "LOW", "NO_CONFIDENCE"}:
            raise RuntimeError("MIN_SOURCE_CONFIDENCE_FOR_HIGH_CONVICTION ma nieprawidłową wartość.")
        if self.min_source_confidence_for_momentum_alert not in {"HIGH", "MEDIUM", "LOW", "NO_CONFIDENCE"}:
            raise RuntimeError("MIN_SOURCE_CONFIDENCE_FOR_MOMENTUM_ALERT ma nieprawidłową wartość.")
